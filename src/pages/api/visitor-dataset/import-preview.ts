import { NextApiRequest, NextApiResponse } from 'next';
import jwt from 'jsonwebtoken';
import formidable from 'formidable';
import fs from 'fs';
import os from 'os';
import Papa from 'papaparse';
import * as XLSX from 'xlsx';

function extractUserFromToken(authHeader?: string) {
  if (!authHeader || !authHeader.startsWith('Bearer ')) {
    throw new Error('Authentication required');
  }

  const token = authHeader.substring(7);
  try {
    const decoded: any = jwt.verify(token, process.env.JWT_SECRET || 'your-secret-key');
    return {
      userId: decoded.userId,
      ownerId: decoded.ownerId,
      username: decoded.username
    };
  } catch (error) {
    throw new Error('Invalid token');
  }
}

// Disable default body parser
export const config = {
  api: {
    bodyParser: false,
  },
};

export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method !== 'POST') {
    return res.status(405).json({ message: 'Method not allowed' });
  }

  try {
    const userInfo = extractUserFromToken(req.headers.authorization);

    // Parse the multipart form data
    const form = formidable({
      uploadDir: os.tmpdir(), // Use OS-specific temp directory
      keepExtensions: true,
      maxFileSize: 100 * 1024 * 1024, // 100MB limit
    });

    const [fields, files] = await form.parse(req);
    
    const uploadedFile = Array.isArray(files.file) ? files.file[0] : files.file;
    
    if (!uploadedFile) {
      return res.status(400).json({ message: 'No file uploaded' });
    }

    const filePath = uploadedFile.filepath;
    const fileName = uploadedFile.originalFilename || '';
    const fileExtension = fileName.split('.').pop()?.toLowerCase();

    let parsedData: any[] = [];
    let headers: string[] = [];

    try {
      if (fileExtension === 'csv') {
        // Parse CSV file
        const fileContent = fs.readFileSync(filePath, 'utf8');
        const parseResult = Papa.parse(fileContent, {
          header: true,
          skipEmptyLines: true,
          preview: 10 // Only preview first 10 rows
        });
        
        if (parseResult.errors.length > 0) {
          console.error('CSV parsing errors:', parseResult.errors);
        }
        
        parsedData = parseResult.data;
        headers = parseResult.meta.fields || [];
        
      } else if (fileExtension === 'xlsx' || fileExtension === 'xls') {
        // Parse Excel file
        const workbook = XLSX.readFile(filePath);
        const sheetName = workbook.SheetNames[0];
        const worksheet = workbook.Sheets[sheetName];
        const jsonData = XLSX.utils.sheet_to_json(worksheet, { header: 1 });
        
        if (jsonData.length < 2) {
          throw new Error('Excel file must have at least a header row and one data row');
        }
        
        headers = (jsonData[0] as string[]).filter(h => h && h.toString().trim());
        
        // Get preview data (first 10 rows)
        const previewRows = (jsonData.slice(1, 11) as any[][]);
        parsedData = previewRows.map(row => {
          const obj: any = {};
          headers.forEach((header, index) => {
            if (header && row[index] !== undefined && row[index] !== null && row[index] !== '') {
              obj[header] = row[index];
            }
          });
          return obj;
        });
      } else {
        return res.status(400).json({ message: 'Unsupported file format. Please upload CSV or Excel files.' });
      }

      // Clean up the uploaded file
      fs.unlinkSync(filePath);

      if (headers.length === 0) {
        return res.status(400).json({ message: 'No headers found in the file' });
      }

      // Available database fields for mapping
      const availableFields = [
        { key: 'fullName', label: 'Full Name', required: true },
        { key: 'email', label: 'Email', required: true },
        { key: 'phoneNumber', label: 'Phone Number', required: true },
        { key: 'company', label: 'Company', required: false },
        { key: 'city', label: 'City', required: false },
        { key: 'state', label: 'State', required: false },
        { key: 'country', label: 'Country', required: false },
        { key: 'pincode', label: 'Pincode', required: false }
      ];

      // Auto-suggest column mappings
      const suggestedMappings: { [key: string]: string } = {};
      
      headers.forEach(header => {
        const normalizedHeader = header.toLowerCase().trim();
        
        // Auto-mapping logic
        if (normalizedHeader.includes('name') && !normalizedHeader.includes('company')) {
          suggestedMappings[header] = 'fullName';
        } else if (normalizedHeader.includes('email')) {
          suggestedMappings[header] = 'email';
        } else if (normalizedHeader.includes('phone') || normalizedHeader.includes('mobile')) {
          suggestedMappings[header] = 'phoneNumber';
        } else if (normalizedHeader.includes('company') || normalizedHeader.includes('organization')) {
          suggestedMappings[header] = 'company';
        } else if (normalizedHeader.includes('city')) {
          suggestedMappings[header] = 'city';
        } else if (normalizedHeader.includes('state')) {
          suggestedMappings[header] = 'state';
        } else if (normalizedHeader.includes('country')) {
          suggestedMappings[header] = 'country';
        } else if (normalizedHeader.includes('pin') || normalizedHeader.includes('zip') || normalizedHeader.includes('postal')) {
          suggestedMappings[header] = 'pincode';
        }
      });

      return res.status(200).json({
        fileName,
        headers,
        previewData: parsedData,
        availableFields,
        suggestedMappings,
        totalRows: parsedData.length,
        message: 'File uploaded successfully. Please map the columns and confirm import.'
      });

    } catch (fileError) {
      // Clean up the uploaded file in case of error
      if (fs.existsSync(filePath)) {
        fs.unlinkSync(filePath);
      }
      throw fileError;
    }

  } catch (error: any) {
    console.error('❌ [Import Preview API] Error:', error);
    
    if (error.message === 'Authentication required' || error.message === 'Invalid token') {
      return res.status(401).json({ message: error.message });
    }
    
    return res.status(500).json({ 
      message: 'Failed to process file',
      details: process.env.NODE_ENV === 'development' ? error.message : 'Please try again'
    });
  }
} 